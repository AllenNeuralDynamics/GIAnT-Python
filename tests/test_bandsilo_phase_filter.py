...
# Preserve the independent scientific reference's explicit sampling loops.
def brute_median(  # noqa: C901
    image,
    selected,
    ref_r,
    ref_c,
    ...
    """Compute a small-grid reference with explicit distance/window loops."""
    ...
    for flat in selected:
        ...
        samples = []
        for sr in range(
            max(0, r - height // 2), min(image.shape[1], r + height // 2 + 1)
        ):
            for sc in range(
                max(0, c - width // 2), min(image.shape[2], c + width // 2 + 1)
            ):
                if abs(
                    phases[z, sr, sc] - phases[z, r, c]
                ) <= tolerance and np.isfinite(image[z, sr, sc]):
    ...
    for flat in selected:
        z, r, c = np.unravel_index(flat, image.shape)
        if (
            not np.isfinite(phases[z, r, c])
            or not np.isfinite(image[z, r, c])
            or np.isfinite(first_med[z, r, c])
        ):
            continue
        window = first_med[
            z,
            max(0, r - height // 2) : r + height // 2 + 1,
...
class TestPhaseMatchedFilter(unittest.TestCase):
    """Check phase membership, spatial support, and frozen fallback medians."""

    def test_overlapping_curved_bands_match_brute_force(self):
        """Match explicit samples across depths, gaps, and chunk sizes."""
        ...
        original = image.copy()
        expected = brute_median(
            image, selected, ref_r, ref_c, ref_d, 9, 7, 0, 3
        )
        for chunk_size in (1, 53, 4096):
            ...
            np.testing.assert_allclose(
                actual, expected, atol=1e-6, equal_nan=True
            )
            self.assertTrue(np.isnan(actual[:, :, 7]).all())
        np.testing.assert_array_equal(image, original)

    def test_signed_phase_not_absolute_distance(self):
        """Keep opposite signed offsets in distinct sample populations."""
    ...
    def test_phase_tolerance_and_minimum_samples(self):
        """Match the reference as tolerance and sample thresholds vary."""
    ...
    def test_midpoint_rows_and_duplicate_centers_are_retained(self):
        """Retain midpoint support while deduplicating reference centers."""
        ...
        np.testing.assert_array_equal(
            med[0, 8:10], 1
        )  # Near ties also retained.

    def test_midpoint_tie_chooses_smaller_reference_row(self):
        """Resolve equidistant references toward the smaller row."""
        image = np.array([[[10.0, 20.0, 30.0, 40.0, 50.0, 100.0, 200.0]]])
        cols = np.arange(7)
    ...
    def test_default_exact_phase_does_not_pool_nearby_offsets(self):
        """Pool nearby phases only when an explicit tolerance permits it."""
    ...
    def test_irregular_spacing_keeps_pixel_offsets_not_fractional_phase(self):
        """Measure phase in pixels rather than local inter-band fractions."""
        ...
        med = si.phase_matched_nanmedian(
            image,
            np.flatnonzero(np.isfinite(image)),
            refs,
            np.tile(cols, 2),
        )
    ...
    def test_fallback_uses_first_pass_medians_not_raw_or_filled_values(self):
        """Freeze first-pass medians so fallback values cannot propagate."""
        ...
        for chunk in (1, 4, 4096):
            ...
            self.assertAlmostEqual(
                float(med[0, 1, 5]), np.mean(expected_med), places=5
            )
    ...
    def test_empty_fallback_is_nan_without_warnings(self):
        """Leave unsupported windows NaN without empty-reduction warnings."""
        import warnings
    ...
    def test_missing_depth_geometry_and_empty_support(self):
        """Require multi-plane geometry and preserve absent-plane NaNs."""
    ...
    def test_finalization_masks_samples_and_preserves_motion_geometry(self):
        """Use shifted geometry and exclude invalid samples at finalization."""
        cols = np.arange(3, 14)
    ...
    def test_invalid_parameters(self):
        """Reject invalid window, chunk, sample, and tolerance settings."""
        image = np.ones((1, 5, 7))
        for options in (
            {"height": 4},
            {"width": 0},
            ...
            {"min_samples": 0},
            {"phase_tolerance": -1},
            {"phase_tolerance": np.inf},
            {"phase_tolerance": np.nan},
        ):
