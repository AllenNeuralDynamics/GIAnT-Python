...
- `params` holds science and optional `num_channels`; `execution` holds `max_workers`, `verbose`, and `max_trials`; `annotations` holds `enabled`, `interactive`, and `operator`. Generic services and `Pipeline` take `microscope` / `scan_mode` as explicit routing options; only SLAP2 band extraction is implemented (`scan_mode=None` also selects band). There is no registration configuration class.
...
The last field is a reference parameter: do not assume every reference parameter is emitted by the band writer. The Python `BandSiloParams.activity_channel` is zero-based; currently only `0` is supported, and source activity arrays for other channels retain NaNs.
