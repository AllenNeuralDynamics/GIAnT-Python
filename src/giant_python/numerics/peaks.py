"""Shared Gaussian peak detection in the activity image.
...
:mod:`giant_python.extraction.band.summary_images`; those seeds initialize the
Phase-6 NMF localization.
...
def _validate_theta(theta: np.ndarray) -> None:
    ...
    if theta.ndim != 2 or theta.shape[1] != 5:
        raise ValueError(
            "Gaussian parameters must have shape (N, 5): "
            "[amp, mu_y, mu_x, sigma_y, sigma_x]; "
            "repeat the width for equal-width Gaussians"
        )
...
def gaussian_peaks_integrated(
    ...
    Parameters
    ----------
    theta : ndarray of shape (N, 5)
        Per-Gaussian ``[amp, mu_y, mu_x, sigma_y, sigma_x]``. Equal widths
        must be supplied explicitly in both width columns.
    ...
    sx = np.maximum(theta[:, 4], np.finfo(float).eps)
...
def _gaussian_peaks_integrated_val_jac(
    ...
    ----------
    theta : ndarray of shape (N, 5)
        Per-Gaussian ``[amp, mu_y, mu_x, sigma_y, sigma_x]``.
    yxdata : ndarray of shape (M, 2)
        Pixel-center ``[y, x]`` coordinates.
    ...
    jac : ndarray of shape (M, 5*N)
        Derivatives in the same order as ``theta.ravel()``.
    """
    _validate_theta(theta)
    x_int = yxdata[:, 1].astype(np.intp)
    y_int = yxdata[:, 0].astype(np.intp)
    ...
    jac = np.empty((m, 5 * n), dtype=float)
    jac[:, 0::5] = dval_da
    jac[:, 1::5] = dval_dmy
    jac[:, 2::5] = dval_dmx
    jac[:, 3::5] = dval_dsy
...
def _lsq_curvefit(
    ...
    *,
    ...
    Parameters
    ----------
    theta0 : ndarray of shape (N, 5)
        Initial ``[amp, mu_y, mu_x, sigma_y, sigma_x]`` per Gaussian.
    ...
    shared_widths : bool
        Constrain width updates to be equal for the isotropic warm start.
        Both initial width columns and their bounds must already match.
    ...
    """
    _validate_theta(theta0)
    if lb_flat.shape != (theta0.size,) or ub_flat.shape != (theta0.size,):
        raise ValueError("Gaussian bounds must have shape (5*N,)")
    if shared_widths and (
        not np.array_equal(theta0[:, 3], theta0[:, 4])
        or not np.array_equal(lb_flat[3::5], lb_flat[4::5])
        or not np.array_equal(ub_flat[3::5], ub_flat[4::5])
    ):
        raise ValueError(
            "Shared-width fitting requires equal y/x widths and bounds"
        )
    x = np.clip(theta0.ravel().copy(), lb_flat, ub_flat)

    val, jac = _gaussian_peaks_integrated_val_jac(x.reshape(-1, 5), xdata)
    r = val - ydata
    cost = np.dot(r, r)
    nfev = 1
    ...
    for _ in range(400):  # MATLAB MaxIter default
        ...
        if shared_widths:
            # Optimize one width degree of freedom, keeping five-column
            # parameters throughout. The chain rule ties the two widths.
            full_jac = jac.reshape(len(r), -1, 5)
            fit_jac = full_jac[:, :, :4].copy()
            fit_jac[:, :, 3] = full_jac[:, :, 3] + full_jac[:, :, 4]
            fit_jac = fit_jac.reshape(len(r), -1)
        else:
            fit_jac = jac
        jtj = fit_jac.T @ fit_jac
        jtr = fit_jac.T @ r
        diag_jtj = np.maximum(np.diag(jtj), 1e-8)

        delta = np.linalg.solve(jtj + lam * np.diag(diag_jtj), -jtr)
        if shared_widths:
            updates = delta.reshape(-1, 4)
            delta = np.column_stack([updates, updates[:, 3]]).ravel()
        x_new = np.clip(x + delta, lb_flat, ub_flat)

        val_new, jac_new = _gaussian_peaks_integrated_val_jac(
            x_new.reshape(-1, 5), xdata
        )
        r_new = val_new - ydata
...
def _fit_isotropic_then_anisotropic(
    ...
    Both stages use five-column parameters. The first stage ties the two
    widths and constrains their shared sigma to the intersection of the y/x
    bounds (normally [0.35, 1] pixels).
    ...
    if np.any(iso_lb[:, 3] > iso_ub[:, 3]):
        raise ValueError(
            "Isotropic fitting requires overlapping y/x width bounds"
        )
    ...
    return _lsq_curvefit(
        isotropic, xdata, ydata, lb_flat, ub_flat, max_nfev=max_nfev
    )
...
def _process_cc_new_peak(
    new_peak: tuple[tuple[int, int], int],
    thetaf: np.ndarray,
    p_locs: np.ndarray,
...
def _refine_residual_peaks(
    ...
    # With output omitted, label returns (label array, component count).
    labeled_full, _ = cast(tuple[np.ndarray, int], ndimage.label(act_sel_pix))
    ...
    while True:
        ...
        labeled_full, _ = cast(
            tuple[np.ndarray, int], ndimage.label(act_sel_pix)
        )
