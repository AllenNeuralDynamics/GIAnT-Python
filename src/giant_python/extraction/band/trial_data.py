...
715-842 and the per-trial low-res caching block ref 1851-1892). Vendor file
construction belongs to shared SLAP2 IO; alignment inputs normalize motion
at the band boundary. The original reader lifetime and accumulation order are
unchanged. Shared interpolation and morphology utilities live in
:mod:`giant_python.numerics`.
"""
...
from ...io.slap2 import open_slap2_file
from .inputs import load_alignment_data_h5
...
def read_band_trial_data(
    ...
    data_file = open_slap2_file(os.path.join(datadr, source_fn))
