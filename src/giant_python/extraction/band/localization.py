...
from :mod:`giant_python.numerics.peaks` and the noise-normalized residual /
per-motion ``H`` matrices from :mod:`giant_python.extraction.band.background`
and :mod:`giant_python.extraction.band.operators`, this
...
def multiplicative_nmf(
    ...
    """
    x_mots: List[torch.Tensor | None] = [None] * n_motions
    shuffled_indices = torch.randperm(n_motions)
    for idx in shuffled_indices:
        # randperm uses an integer dtype, so item() returns a Python int.
        i = cast(int, idx.item())
        frames = _motion_frames(mot_inds_yx, i)
        ...
        x_mots[i] = x
    # randperm visits every slot exactly once, including the empty case.
    return cast(List[torch.Tensor], x_mots)
...
def fit_gaussian_profiles(
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % n_motions == 0:
            shuffled_indices = torch.randperm(n_motions)
        # Epoch zero initializes the permutation before any index is read.
        i = cast(
            int,
            cast(torch.Tensor, shuffled_indices)[epoch % n_motions].item(),
        )
        ...
        if epoch % n_motions == n_motions - 1:
            ...
            # This branch completes a nonempty motion round: loss is Tensor.
            losses.append(cast(torch.Tensor, total_loss).item())
            if (
                epoch // n_motions > 0
...
def fit_sources(
    ...
    ----------
    source_seeds : ndarray of shape (N, 3)
        Peak seeds ``[z, mu_y, mu_x]`` from :mod:`giant_python.numerics.peaks`.
