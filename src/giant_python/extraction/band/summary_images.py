...
* :func:`accumulate_activity_image` / :func:`finalize_activity_image` (ref
  2358-2504) turn the smoothed ``rho`` from
    :mod:`giant_python.extraction.band.activity` into the ``act_im`` activity
    image that Phase-5 peak detection localizes sources in.
...
from ...numerics.morphology import fast_dilation
...
def compute_mean_image(
    ...
    num_channels: int,
    low_res_data2_norm: np.ndarray | None = None,
) -> np.ndarray:
    ...
    unique_motion : ndarray of shape (n_bins, 3)
        ...
        :func:`giant_python.extraction.band.motion_binning.bin_motion`.
    mot_inds : ndarray
    ...
    if num_channels >= 2:
        # The two-channel input contract requires the second-channel array.
        mean_im[1, ref_d, sel_r, sel_c] = np.nanmean(
            cast(np.ndarray, low_res_data2_norm)[:, frames_to_keep], axis=1
        )
    return mean_im
...
def accumulate_activity_image(
    ...
    for batch_idx in progress(
        ...
        br[time_indices, depth_indices, row_indices_roi, col_indices_roi] = (
            batch_vals
        )
        nm[time_indices, depth_indices, row_indices_roi, col_indices_roi] = (
            np.isnan(batch_vals)
        )
...
def finalize_activity_image(
    ...
    nan_ct : ndarray of shape (n_sel,)
        ...
        :func:`giant_python.extraction.band.activity.mask_high_nan_rho`.
    ...
    nan_mask[np.unravel_index(sel_pix_idxs[valid_sel_pix], nan_mask.shape)] = (
        False
    )
    ...
    # With no output override, ndimage preserves the boolean input dtype.
    vertical_interior = cast(
        NDArray[np.bool_],
...
# Preserve scientific parity of phase assignment and frozen fallback passes.
def phase_matched_nanmedian(  # noqa: C901
    image: np.ndarray,
    ...
    if image.ndim != 3 or any(size == 0 for size in image.shape):
        raise ValueError(
            "image must have non-empty shape (depth, row, column)"
        )
    ...
    for coordinates in (ref_c, ref_d):
        if not np.all(np.isfinite(coordinates)) or np.any(
            ...
            raise ValueError(
                "reference columns and depths must be finite integers"
            )
    ref_c, ref_d = ref_c.astype(np.intp), ref_d.astype(np.intp)

    selected = np.asarray(valid_pix_idxs)
    if selected.ndim != 1 or (
        selected.size and not np.issubdtype(selected.dtype, np.integer)
    ):
        raise ValueError(
            "valid_pix_idxs must be a one-dimensional integer array"
        )
    ...
    result = np.full(
        image.shape, np.nan, dtype=np.result_type(image.dtype, np.float32)
    )
    ...
    references = {}
    for depth, column, row in zip(
        ref_d[in_bounds], ref_c[in_bounds], ref_r[in_bounds]
    ):
    ...
    boundaries = np.r_[
        0, np.flatnonzero(np.diff(keys[order])) + 1, selected.size
    ]
    ...
    for start, stop in zip(boundaries[:-1], boundaries[1:]):
        ...
        nearest = np.where(
            np.abs(pixel_rows - centers[left])
            <= np.abs(pixel_rows - centers[right]),
            left,
            right,
        )
    ...
    def windows(output_positions):
        """Yield chunked spatial lookups and exact selected-pixel support."""
        for start in range(0, output_positions.size, chunk_size):
            ...
            sample_flat = (
                depths[positions, None] * nr + sample_r
            ) * nc + sample_c
            lookup = np.minimum(
                np.searchsorted(selected, sample_flat), selected.size - 1
            )
    ...
    for positions, lookup, support in windows(outputs):
        accept = (
            ...
            & (
                np.abs(phase[lookup] - phase[positions, None])
                <= phase_tolerance
            )
        )
        ...
        if not np.any(enough):
            continue
        samples = np.where(
            accept[enough], values[lookup[enough]], np.nan
        ).astype(result.dtype)
    ...
    for positions, lookup, support in windows(missing):
        ...
        totals = np.sum(
            np.where(accept, first_medians[lookup], 0),
            axis=1,
            dtype=np.float64,
        )
        result.ravel()[selected[positions[enough]]] = (
            totals[enough] / counts[enough]
        )
    return result
...
def snake_aligned_nanmedian(
    ...
    for segment, (start, stop) in enumerate(zip(run_starts, run_stops)):
        ...
        for index in range(start + 1, stop):
            ...
            center_rows[index] -= (
                np.rint(delta / snake_spacing) * snake_spacing
            )
    ...
    result = np.full_like(
        image, np.nan, dtype=np.result_type(image, np.float32)
    )
    ...
    for start in range(0, len(valid_pix_idxs), chunk_size):
        ...
        valid_cols &= (
            trajectory_segment[clipped_cols] == trajectory_segment[cols, None]
        )
        path_delta = np.rint(
